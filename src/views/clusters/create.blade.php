  ...
  <div class="col-md-8 col-md-offset-2">
    <div class="login-panel panel panel-default">
      <div class="panel-heading">
          <h3 class="panel-title">{{trans('mysql-cluster::clusters.create')}}</h3>
      </div>
      <div class="panel-body" ng-controller="ClusterController as cluster" ng-app="clusters">
          @if(Session::has('error-message'))
          <div class="alert alert-danger">
            {{Session::get('error-message')}}
          </div>
          @endif
          {{Form::open(['url'=>route('mysql-admin.clusters.store'),'method'=>'post'])}}
          <fieldset>
              <div class="form-group">
                  <input class="form-control" placeholder="name" name="name" type="text" autofocus="">
              </div>
              <div class="form-group">
                  <input class="form-control" placeholder="database name" name="database" type="text" autofocus="">
              </div>
              <div class="hosts" ng-repeat="(key, config) in cluster.clusters">
                ...
                <div class="form-group">
                    <input class="form-control" placeholder="host" name="config[@{{key}}][host]" ng-value="config.name">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label>
                      <input type="checkbox" ng-change="cluster.releaseDefault(key);" name="config[@{{key}}][default]" ng-model="config.default" > Set Default
                    </label>
                </div>
              </div>
